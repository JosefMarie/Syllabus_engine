...
export default function SidebarTree({
  ...
  const [expandedLOs, setExpandedLOs] = useState<Record<string, boolean>>({
    [syllabus.learningOutcomes[0]?.id || ""]: true,
  });
  const [expandedICs, setExpandedICs] = useState<Record<string, boolean>>({
    [syllabus.learningOutcomes[0]?.indicativeContents[0]?.id || ""]: true,
  });
  const [expandedTopics, setExpandedTopics] = useState<Record<string, boolean>>({
    [syllabus.learningOutcomes[0]?.indicativeContents[0]?.topics[0]?.id || ""]: true,
  });
    ...
    <div className="flex h-full flex-col bg-[#1E293B] text-[#CBD5E1]">
        ...
        <div className="flex items-center justify-between">
          <Link href="/" className="flex items-center space-x-2 text-white hover:text-[#06B6D4] transition-colors">
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#06B6D4] text-xs font-bold font-mono text-slate-950">
              {syllabus.courseCode.slice(0, 3)}
            </span>
            <span className="text-sm font-bold font-mono tracking-tight">{syllabus.courseCode}</span>
          </Link>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {syllabus.learningOutcomes.map((lo, loIdx) => {
          const isLOOpen = expandedLOs[lo.id] || searchQuery.length > 0;
          return (
              ...
              >
                <div className="flex items-center space-x-2 min-w-0">
                  ...
                  <span className="text-xs font-bold text-white truncate">
                    <span className="text-[#06B6D4] font-mono mr-1.5">{lo.code}:</span>
                    {lo.title}
                  </span>
                </div>
              </button>
              ...
              {isLOOpen && (
                <div className="border-t border-[#334155]/40 pl-2 pr-1 py-1 space-y-1.5">
                  {lo.indicativeContents.map((ic) => {
                    const isICOpen = expandedICs[ic.id] || searchQuery.length > 0;
                    return (
                      <div key={ic.id} className="ml-2 rounded-lg border-l-2 border-[#06B6D4]/40 bg-[#1E293B]/40">
                        ...
                        >
                          <div className="flex items-center space-x-2 min-w-0">
                            ...
                            <span className="text-xs font-semibold text-[#CBD5E1] truncate">
                              <span className="text-[#F59E0B] font-mono mr-1">{ic.code}:</span>
                              {ic.title}
                            </span>
                          </div>
                        </button>
                        ...
                        {isICOpen && (
                          <div className="pl-3 pr-1 py-1 space-y-1">
                            {ic.topics.map((top) => {
                              const isTopicOpen = expandedTopics[top.id] || searchQuery.length > 0;
                              return (
                                    ...
                                    <div className="mt-1 space-y-0.5 pl-2">
                                      {top.subtopics
                                        ...
                                        .map((sub) => {
                                          ...
                                          const isCompleted = progressMap[sub.id];
